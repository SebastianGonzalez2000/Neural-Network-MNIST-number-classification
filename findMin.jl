...
function findMinNN(funObj,w;maxIter=100,epsilon=1e-2,verbose=true)
	# funObj: function that returns (objective,gradient)
	# w: initial guess
	# maxIter: maximum number of iterations
	# epsilon: stop if the gradient gets below this

	# Set negative values of parameters to zero
	w = max.(0,w)
...
function findMinL1(funObj,w,lambda;maxIter=100,epsilon=1e-2,verbose=true)
	# funObj: function that returns (objective,gradient)
	# w: initial guess
